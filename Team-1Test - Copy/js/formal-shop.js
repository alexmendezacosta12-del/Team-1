  ...
  card.innerHTML = `
    ...
    <div class="f-product-info">
      ...
      <div class="f-product-footer">
        ...
        <button class="f-btn-view-sizes" data-product-id="${product.id}">Select Size</button>
      </div>
    </div>
  `;

  card.querySelector(".f-btn-view-sizes").addEventListener("click", () =>
    formalShowSizeSelection(product)
  );
  return card;
}
...
function formalShowCheckout() {
  ...
  if (modal) {
    ...
    if (summaryEl) {
      summaryEl.innerHTML = formalCart.items
        .map(
          (item) =>
            `<div class="f-checkout-item">
               <span>${item.name} (UK ${item.size}) × ${item.quantity}</span>
               <span>£${(item.price * item.quantity).toFixed(2)}</span>
             </div>`
        )
        .join("") +
        `<div class="f-checkout-total">
           <strong>Total</strong>
           <strong>£${formalCart.getTotalPrice().toFixed(2)}</strong>
         </div>`;
    }
    modal.classList.add("active");
  }
    ...
    container.innerHTML = this.items
      ...
      <div class="f-cart-item">
        ...
        <div class="f-cart-item-controls">
          ...
          <p class="f-cart-item-price">£${(item.price * item.quantity).toFixed(2)}</p>
