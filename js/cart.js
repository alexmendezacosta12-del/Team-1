...
class ShoppingCart {
  ...
  // Get total price (raw, before any discount)
  getRawTotal() {
  ...
  renderCartModal() {
    ...
    container.innerHTML = this.items
      .map(
        (item) => {
          ...
          const linePrice = isLoggedIn ? (typeof getDisplayTotal === 'function' ? getDisplayTotal(lineRaw) : lineRaw) : lineRaw;
          return `
            <div class="cart-item">
                ...
                <div class="cart-item-controls">
                    ...
                    <p class="cart-item-price">£${linePrice.toFixed(2)}</p>
    ...
    const savingsHtml = savings > 0
      ? `<div class="cart-savings-row"><span class="cart-savings-label">Member saving (10%)</span><span class="cart-savings-amount">−£${savings.toFixed(2)}</span></div>`
      : '';
    ...
    // Insert savings row before total row if logged in
    const footer = document.querySelector('.cart-footer');
    const existingSavings = document.getElementById('cart-savings-dynamic');
    if (existingSavings) existingSavings.remove();
    if (savings > 0 && footer) {
      const savingsEl = document.createElement('div');
      savingsEl.id = 'cart-savings-dynamic';
